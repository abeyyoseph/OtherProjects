...
class AtomWallEnv(atom_env.AtomEnv):
    ...
    def discretizePoint(self, x, y, grid_size):
        discretized_x = round(x / grid_size) * grid_size
        discretized_y = round(y / grid_size) * grid_size
    ...
    def _init_env_variables_custom(self, goal_x, goal_y):
        ...
        :return:
        """
        self.desired_point.x = goal_x
        self.desired_point.y = goal_y
        ...
        # self.desired_point_discretized.x, self.desired_point_discretized.y = self.discretizePoint(goal_x, goal_y, self.grid_size)
    ...
    def _get_obs(self):
        ...
        odometry_original = [round(x_position / 1.0), round(y_position / 1.0)]
        ...
        # print("Relative goal position: " + str(relative_position_to_goal[0]) + ","+ str(relative_position_to_goal[1]))

        # The observation is the discretized lidar data and the x,y position of the robot
        # observations = discretized_laser_scan_normalized + odometry_array_normalized + relative_position_to_goal
        
        
        relative_position_to_goal = self.calculate_relative_position(x_position, y_position, self.desired_point.x, self.desired_point.y)
    ...
    def is_near_obstacle(self, current_position, epsilon=0.05):
        """
    ...
    def is_in_desired_position(self, current_position, epsilon=0.05):
        ...
        # x_pos_plus = self.desired_point_discretized.x + epsilon
        # x_pos_minus = self.desired_point_discretized.x - epsilon
        # y_pos_plus = self.desired_point_discretized.y + epsilon
        # y_pos_minus = self.desired_point_discretized.y - epsilon
    ...
    def calculate_heading_difference(self, current_odometry):
        ...
        # Goal position
        goal_x = self.desired_point.x  
        goal_y = self.desired_point.y 
