...
if __name__ == '__main__':
    ...
    rospy.init_node('atom_wall_n1try', anonymous=True, log_level=rospy.WARN)
    ...
    n1try = n1try.N1try()
    n1try.loadModelState("/home/abey/Desktop/Repos/Data_Analysis_Scripts/KSU_GradSchool/MTRE6800_ResearchProject/SavedNNParameters/DQN/")
    ...
    for x in range(START, nepisodes+START):
        n1try.current_episode += 1  # Increment episode counter
        if n1try.current_episode % 30 == 0:
            n1try.update_learning_rate()
        ...
        for i in range(nsteps):
            ...
            action = n1try.chooseAction(state, Epsilon, action_space)
            ...
            nextState = n1try.preprocess_state(nextState)
            n1try.learn(state, action, reward, nextState, done)
