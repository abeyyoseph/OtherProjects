...
bool BotDrive::init()
{
  ...
  pose_sub = nh_.subscribe("pose", 1, &BotDrive::pose_callback, this);
...
void BotDrive::pose_callback(const geometry_msgs::PoseStampedPtr &pose)
{
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "/atom/odom", "/wheeled_robot/chassis"));
    std::cout << "Sent transform!" << std::endl;
